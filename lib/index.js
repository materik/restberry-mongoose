...
var RestberryORM = require('restberry-orm');


function RestberryORMMongoose() {
    this.mongoose = mongoose;
};

RestberryORMMongoose.prototype.__proto__ = RestberryORM.prototype;

RestberryORMMongoose.prototype.addMethods = function(schema, methods) {
...
RestberryORMMongoose.prototype.addStatics = function(schema, statics) {
...
RestberryORMMongoose.prototype.connect = function(dbname) {
    mongoose.connect(dbname);
};

RestberryORMMongoose.prototype.get = function(name) {
    try {
...
RestberryORMMongoose.prototype.schema = function(schema) {
    var schema = new mongoose.Schema(schema);
    this.addMethods(schema, methods);
    this.addMethods(schema, {
        restberry: this.restberry,
        onError: this.restberry.web.handleRes,
    });
    this.addStatics(schema, statics);
    this.addStatics(schema, {
        restberry: this.restberry,
        onError: this.restberry.web.handleRes,
    });
    return schema;
...
RestberryORMMongoose.prototype.set = function(name, schema) {
    return mongoose.model(name, schema);
};

RestberryORMMongoose.prototype.use = function(next) {
    if (next)  next(this);
};
...
var methods = {
    ...
    removeAndVerify: function(req, res, next) {
        ...
        self._isAuthorized(req, res, function() {
            self.remove(function(err) {
                if (err) {
                    new errors.BadRequest(err, req, res, self.onError);
                } else {
                    next();
                }
            });
            ...
            self.save(function(err, obj) {
                if (err) {
                    ...
                    if (_s.include(err.message, 'E11000')) {
                        ...
                        new errors.Conflict(err, req, res, self.onError);
                    } else {
    ...
    isAuthorized: function(req, res, next) {
        var self = this;
        var User = self.restberry.auth.User.model;
        if (User) {
            self.constructor.getFieldNameOfModel(User, function(fieldName) {
                var fieldId = null;
            ...
            new errors.ServerIssue({}, req, res, self.onError);
        }
    },
        ...
        if (req.loginRequired) {
            var done = function(isAuthorized) {
                if (isAuthorized) {
                    ...
                    console.log(errors.Forbidden)
                    new errors.Forbidden({}, req, res, self.onError);
                }
            };
            if (req.method === 'POST' && self.isCreateAuthorized) {
                self.isCreateAuthorized(req, res, done);
            } else if (req.method === 'GET' && self.isReadAuthorized) {
                self.isReadAuthorized(req, res, done);
            } else {
                self.isAuthorized(req, res, done);
            }
        } else {
            next();
        }
...
var statics = {
    ...
    findByIdAndVerify: function(req, res, id, next) {
        ...
        self.findById(id, function(err, obj) {
            var _notFound = function() {
                ...
                new errors.NotFound(err, req, res, self.onError);
            }
            if (err) {
    ...
    validateData: function(req, res, data, next) {
        ...
        if (illegalFields.length) {
            ...
            new errors.BadRequest(err, req, res, this.onError);
...
module.exports = exports = new RestberryORMMongoose;
